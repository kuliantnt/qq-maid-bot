...
use chrono::{
    DateTime, Datelike, Duration, FixedOffset, NaiveDate, NaiveDateTime, SecondsFormat, Timelike,
    Utc,
};
use regex::Regex;
...
impl RequestTimeContext {
    ...
    pub fn llm_prompt(&self) -> String {
        format!(
            ...
            self.current_date, self.current_time, self.timezone
        )
    }
...
mod tests {
    ...
    #[test]
    fn formats_request_time_context_for_prompts() {
        ...
        assert!(ctx.llm_prompt().contains("不要自行猜测当前日期"));
    }
