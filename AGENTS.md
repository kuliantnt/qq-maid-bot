...
- 单文件达到约 1000 行时，应检查是否存在可独立命名的职责边界；1000 行是触发架构审查的信号，不是强制验收阈值，不得仅为满足行数限制机械移动代码。
- `mod.rs` 优先作为模块门面、核心类型和子模块编排入口，不应单纯为了规避行数限制承载接近 1000 行的混合实现。
- 避免使用语义模糊的大型 `helpers.rs` / `utils.rs` 承接拆出的杂项；逻辑可以按领域职责命名时，应优先使用 `session.rs`、`bootstrap.rs`、`addressing.rs` 等明确名称。
- 不是所有 700～900 行文件都必须继续拆分；只有职责明显混杂时才需要进一步整理。强相关逻辑可以保留在同一文件中，不要为了数字指标破坏局部可读性。
- 函数尽量不要超过 100 行，单个文件不要超过 3 个职责模块；需要拆分时优先采用 `mod.rs` + 职责明确的子模块。

### 日志规范

- 项目自有日志的可读正文统一使用中文。
- 结构化字段名、错误分类、状态值和协议标识使用稳定的英文标识。
- 第三方及上游错误应保留原始内容，不得仅保留翻译后的文本。
- 日志优先使用结构化字段，不要将关键上下文全部拼接进文案。
- 高频轮询、心跳和空闲状态不得使用 `debug` 或更高级别持续刷屏，必要时使用 `trace`。
