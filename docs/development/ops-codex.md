...
`/ops codex <任务描述>` 允许受信任的机器人管理员向服务器上的 Codex CLI 提交长任务。任务在固定工作目录和固定沙箱中异步执行，受理后立即返回任务 ID；运行超过 45 秒时会通过 Notification Outbox 推送低频进度，最终结果仍推送回原会话。
...
NVM 版 Codex 的脚本通常通过 `/usr/bin/env node` 启动，而 systemd 服务的 `PATH` 往往不包含 NVM 目录。`/ops codex` 会把规范化后的固定 `program` 父目录放到 Codex 子进程的 `PATH` 最前面，因此 `node` 和 `codex` 位于同一 `bin` 目录时，无需修改机器人进程的全局 `PATH`。该行为不影响普通 `/ops` 固定命令。

Codex/Node 应安装在工作区之外，例如独立的 NVM `versions/<node版本>/bin` 目录。使用 `workspace-write` 时，配置加载会规范化现有路径并拒绝 `program` 父目录位于 `working_directory` 内的配置，包含通过 `..` 或符号链接落入工作区的情况；无法安全确认路径关系时也会拒绝启动。
...
发布运行目录通常不带 `.git`。入口固定使用 `codex exec --skip-git-repo-check --ephemeral`，因此这种目录可以正常执行且不会持久化本次 Codex session；跳过 Git 仓库检查不会放宽 `working_directory`、profile 或 sandbox。
...
任务在 45 秒内结束时不会多发过程通知；超过 45 秒仍在运行时发送第一条，此后每 120 秒最多一条。进度只包含任务 ID、仍在运行、已运行时间和取消命令，不包含原始 stdout/stderr、命令参数、任务描述或文件内容。每条进度使用 `ops:<任务ID>:progress:<序号>` 独立去重，Outbox 重试只重发该快照，不会重新执行 Codex。任务结束时会停止定时器并取消尚未终结的进度，再排入原有 `ops:<任务ID>:result` 最终结果。

任务结束后会收到成功、执行失败、超时、取消或启动失败的真实状态。模型文字不能替代进程退出状态。Codex 正常执行时会把进度、插件告警和工具日志写入 stderr；成功任务只向聊天推送最终 stdout。失败、超时或取消时，stderr 会写入权限受限的 `logs/ops/<任务ID>.log`，聊天只提示服务器日志路径。写入失败时，最终结果会直接附带经过任务描述字面替换且限制长度的 stderr 摘要，不会错误提示查看没有详情的主日志。普通 `/ops` 固定命令仍按原样返回 stdout 和 stderr。
...
`/ops list` 只展示当前机器人进程中仍在运行的任务。首期任务注册表不跨进程恢复；机器人重启后，重启前仍在执行的任务不会继续收集结果，启动时会取消其尚未终结的旧进度快照，避免误报“仍在运行”。
...
查看回执中的退出码，再按提示到服务器读取 `logs/ops/<任务ID>.log`；若独立日志写入失败，则查看回执中的有限 stderr 摘要。常见原因包括 Codex profile 不存在、认证不可用、模型服务失败或任务内命令返回非零。框架不会把 stdout 中的“成功”字样当作成功状态。错误日志不会自动上传或提交，需要由部署者按本机日志策略清理。
...
- 错误诊断会替换任务描述完整文本及其非空行的字面匹配，但不能保证识别转义、变形或模型改写后的敏感内容；任务描述本身仍不得包含凭证或隐私数据。
