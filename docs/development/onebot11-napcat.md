...
以下字段名以当前 NapCat WebUI 的**网络配置 → 反向 WebSocket 客户端**表单为准（对应源码 `packages/napcat-webui-frontend/src/components/network_edit/ws_client.tsx`）。NapCat 不再分 Host / Port / Path / SelfID 多个输入框，反向 WS 只填一个 URL，自身账号 ID 由 NapCat 自动上报。
...
在 NapCat 的网络配置中新增一条**反向 WebSocket 客户端**，按下表填写：

| NapCat WebUI 字段 | 建议取值 | 说明 |
| --- | --- | --- |
| 启用 | 开启 | 保存后才会启用此网络配置，因此填完所有字段后再打开，或保存后回来勾选 |
| 名称 | 随意，例如 `qq-maid-bot` | 仅 NapCat 内部标注，不影响连接 |
| URL | `ws://127.0.0.1:8789/onebot/v11/ws` | 单一 URL 字段，由 host + port + path 拼成；跨主机或经 TLS 反代时改用对应 `ws://` / `wss://` 地址 |
| 上报自身消息 | 关闭 | 避免机器人自启发消息被本项目再次送入 Core 形成回声循环；本项目同样在 adapter 层过滤 `message_sent` 与 `user_id == self_id` |
| 消息格式 | **Array** | 必须使用消息段数组；选 `string`（CQ 码）会被本项目忽略 |
| Token | 与 `ONEBOT11_ACCESS_TOKEN` **逐字相同** | **只填原始 Token，不要带 `Bearer ` 前缀**；NapCat 会自动用 `Authorization: Bearer <token>` 发起握手 |
| 心跳间隔 | 保持 NapCat 默认（30000ms） | NapCat 据此周期发 heartbeat；本项目按其 interval 的 2 倍（不小于 `ONEBOT11_REQUEST_TIMEOUT_MS`）做超时监督 |
| 重连间隔 | 保持 NapCat 默认（30000ms） | NapCat 断线后按此间隔自动重连，本项目无需配置重连 |
| SSL 证书验证 | 使用正常 `wss://` 证书时保持开启；仅自签名测试环境才考虑关闭 | 仅 `wss://` 连接生效；关闭会降低安全性，生产环境应保持开启 |

URL 组成对照（NapCat 不需要分别填写，仅供理解）：

- host：`127.0.0.1`，对应本项目 `ONEBOT11_BIND_HOST`
- port：`8789`，对应 `ONEBOT11_BIND_PORT`
- path：`/onebot/v11/ws`，对应 `ONEBOT11_WEBSOCKET_PATH`
- 跨主机示例：`ws://bot-host:8789/onebot/v11/ws`；经反代 TLS 后：`wss://bot.example.com/onebot/v11/ws`

NapCat 会自动以登录 QQ 号（`selfInfo.uin`）作为 `X-Self-ID` 请求头发送，**无需手填 SelfID**。

保存后请确认该网络配置的“启用”开关已打开，否则 NapCat 不会发起连接。连接成功的判定：

- 启动日志中出现 `OneBot 11 client connected`，`/ping` 中 OneBot 状态变为已连接并展示脱敏 `self_id`。
- NapCat 端给出连接成功提示，并按其配置的“心跳间隔”周期发送 heartbeat。
...
2. `X-Self-ID` 请求头：NapCat 会自动以登录 QQ 号（`uin`）发送该头，无需手填。本项目先校验它是非空十进制 ID；未带该头时，本连接会等到首条事件携带的 `self_id` 再完成注册，超出 `ONEBOT11_REQUEST_TIMEOUT_MS` 仍未上报则关闭连接。
...
- **群聊**：默认只在消息里出现指向本机器人 `self_id` 的 `at` 段时进入 Core。回复通过 `send_group_msg` 发到原 group_id。
  ...
  - `at` 段**不会**拼入送进 Core 的正文：指向机器人的 `at` 只用于触发；对其他成员的 `at` 作为结构化 `mentions` 信息保留，不并入纯文本正文。
...
| 现象 | 排查方向 |
| --- | --- |
| NapCat 一直连不上，本项目日志显示 `rejected unauthorized OneBot 11 WebSocket connection` | 两边 Token 不一致。NapCat 的 Token 字段应只填与 `ONEBOT11_ACCESS_TOKEN` 逐字相同的原始值，**不要带 `Bearer ` 前缀**（NapCat 会自动拼成 `Authorization: Bearer <token>`）；同时检查有无多余空格或换行 |
