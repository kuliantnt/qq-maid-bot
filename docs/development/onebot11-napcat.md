...
- 回复可以是纯文本，也可发送标准 OneBot `image` 图片段
- 待办每日提醒、RSS 推送能发到 QQ 私聊或群

**暂不能：**

- 一个小女仆进程同时连接多个 QQ 账号（要多个账号得多开进程）
- 发文件、Markdown、平台原生引用、`@` 消息段、流式输出或图片之外的其他富媒体回复；图片已支持标准 OneBot `image` segment
...
| fake OneBot | 自动化集成测试 | 覆盖私聊、群聊 `@`、引用机器人免 `@`、引用其他成员不误触发、媒体/未知段降级、文本回复与主动推送，以及连接和 action 异常边界 |
