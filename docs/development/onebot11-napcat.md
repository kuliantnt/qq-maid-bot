...
- 群里要 `@` 她才回；引用当前进程内 ref_index 可命中的机器人消息也可以免 `@` 继续追问，私聊直接说就行
- 接收有序的文本、图片、文件和未知消息段；安全的远程图片可进入图片理解，文件和不可读媒体会明确降级为摘要，但不解析文件正文
- 普通回复可以是纯文本，也可发送标准 OneBot `image` 图片段；群主动推送可额外使用标准 `at` segment 提醒成员
- 待办每日提醒、RSS 推送能发到 QQ 私聊或群

**暂不能：**

- 一个小女仆进程同时连接多个 QQ 账号（要多个账号得多开进程）
- 普通回复仍不发送文件、Markdown、平台原生引用、`@` 消息段、流式输出或图片之外的其他富媒体；群主动推送的成员提醒不属于普通回复，使用标准 `at` segment
- 普通群消息不 `@` 也响应（引用她的进程内可命中消息除外；直接发 `/todo` 仍需先 `@`）
...
- **引用追问**：引用当前进程运行期间由她发出、且仍能被 ref_index 命中的消息时，可以不 `@` 继续追问。ref_index 是进程内缓存；重启后引用旧消息会安全 miss，不会误触发机器人。
- `@` 她的这一下只用来“敲门”，不会把 `@女仆` 这几个字混进她真正看到的正文里；你消息里 `@` 了别人，那个 `@` 也只会作为“提到了谁”的结构信息保留，不会污染正文。

## 验证范围与证据

这里把“自动化测过”和“真实环境用过”分开记录，避免把协议推断写成兼容结论：

| 对象 | 证据等级 | 当前结论 |
| --- | --- | --- |
| NapCat | 维护者实机验证 | 维护者已在真实 NapCat 环境完成一期验证；[#444](https://github.com/kuliantnt/qq-maid-bot/issues/444) 未提供逐项人工测试记录，因此本文不扩写具体项目或结果 |
| fake OneBot | 自动化集成测试 | 覆盖私聊、群聊 `@`、引用机器人免 `@`、引用其他成员不误触发、媒体/未知段降级、文本回复、群主动推送 `at` segment 以及连接和 action 异常边界 |
| 其他 OneBot 11 实现 | 未验证 | 不声明实机兼容；一期协议范围仅为反向 WebSocket 和 Array 消息格式 |

仓库现有自动化测试还覆盖失败 retcode、action 超时、乱序或未知 echo、超大帧、非法 JSON、未知事件、心跳与无心跳客户端、断线重连、同账号连接替换、不同账号连接拒绝、shutdown 清理，以及 ref_index 跨账号/会话隔离。图片、文件、未知 segment、不可读或超限媒体和本地路径降级也已有映射测试。本次发布收口只更新能力状态测试，不重复增加上述行为测试。
...
- 普通回复时带平台引用、`@`、图片或文件（群主动推送成员提醒已支持 `at` segment）
- 群里不 `@` 也响应、特定群免 `@`
- 一个女仆同时连多个 QQ 账号
- 群成员变动等通知类事件做业务

想了解这些后续方向的开发者，可以从归档的 [设计边界](../tasks/archive/onebot11-connect.md) 看起。
