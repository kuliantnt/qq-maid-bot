...
当前稳定版本线为 `23.x`（`v0.23.1`）；发布变更与升级边界见 [CHANGELOG.md](../CHANGELOG.md)。
...
- `qq-maid-core/`：Rust Core / 查询 / 记忆 / session / prompt / 业务 Tool 模块，通过 `CoreService` 提供进程内业务入口；HTTP 层固定公开 `GET /healthz`，启用部署管理控制台时再注册认证、配置和受保护的资源管理路由。
