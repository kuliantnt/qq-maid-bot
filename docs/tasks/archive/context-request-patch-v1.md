...
>
> #166 后更新：通用 Core 已移除成员映射和成员身份上下文，本文件的当前架构描述不再包含这些来源。
...
* `PromptConfig::load_chat_system_prompts(user_text)` 已在普通聊天链路接入上下文模块，
  顺序为：固定 prompt → WORLD_FILE → 上下文模块。
...
pub enum CachePolicy {
    /// 长期稳定，适合作为缓存前缀。典型：maid_system、mode_rules、world。
    Stable,
    /// 会话内相对稳定。典型：session_summary。
    Session,
...
pub struct ContextEntry {
    pub id: String,
    pub source: &'static str,        // "prompt_dir" / "world" / "context_module" / "memory" / "session" / "request_time"
...
* 现有 `PROMPT_DIR`、`WORLD_FILE` 不改名、不迁移；
