...
> 归档状态：该早期分阶段方案已被通用 Agent Loop、服务端 Tool Registry、业务 Tool 领域边界和统一 diagnostics / cancel 语义取代。当前维护边界见 [开发维护文档](../../DEVELOPMENT.md#agent-chat-与-tool-边界) 和 [自定义 Tool 指南](../../development/custom-tools.md)。
...
* 写入类工具必须复用 Core 既有执行与 pending 边界；
* 写入工具需要用户确认时，必须返回结构化 `confirmation_required` 状态；新增 Todo 这类直接执行工具必须返回真实持久化结果；
* 收到 `confirmation_required` 后，当前 Harness 必须立即停止，不继续调用其他工具；
* 最终回复必须基于真实工具结果：需要确认时只能说明“已创建待确认操作”，直接执行成功时才可说明写入已经完成；
...
* 写入需要确认时，最终回复只说明已创建待确认操作；直接执行的 Todo 新增必须基于真实工具结果说明已写入；
