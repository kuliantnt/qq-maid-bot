...
1. 短命令先由 Core 包装成事件流：`CommandStarted` -> `CommandFinished` -> `Completed`；失败时允许 `CommandStarted` 后直接 `Failed`，不得伪造完成状态。
2. 长耗时命令再补 `Status` 进度，如 RSS 拉取、WebSearch、外部查询。
3. 命令业务函数仍返回 `RespondResponse`，先由外层 adapter 转成事件，等稳定后再决定是否让命令内部直接产出事件。

当前仅 `/help` / `/帮助` 作为短命令试点进入 `RespondPlan::CommandEvent`。`/天气`、`/rss`、`/train` 等其它 slash command 仍保留完整响应路径，不应把本文档理解为所有 slash command 已完成迁移。
...
- 已先在外层包装 `/help` / `/帮助` 作为短命令试点，不改各命令业务函数。
- 长耗时命令按需增加进度事件。
- pending 确认继续保持确定性路径。
...
- `/help` 成功路径输出 `CommandStarted` -> `CommandFinished` -> `Completed`，失败路径输出 `CommandStarted` -> `Failed`，且事件包装不改变用户可见最终正文。
- `/天气`、`/rss`、`/train` 等未迁移 slash command 仍保持完整响应路径。
- 超长最终正文仍由 Gateway 安全分片。
- ref_index 记录最终实际发送正文，不记录处理中状态。

## 当前未解决问题

- 当前只有 `/help` / `/帮助` 通过外层 `CommandEvent` adapter 试点；迁移更多命令前仍需评估 `CommandDispatcher` 的最小改造点。
