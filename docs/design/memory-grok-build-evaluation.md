...
- `MEMORY_DREAM_MIN_NEW_SESSIONS`：第一条触发路径所需的最少新增 Session，默认 5；该路径还要求检查点后至少 30 条用户消息；
...
同一 target 通过基础冷却后，首次批次要求检查点后的有效用户消息满足以下任一路径：

- 不同 Session 至少 5（或配置值），且消息至少 30 条；
- 按 `Asia/Shanghai` 换算后不同活跃日期至少 3 天，且消息至少 50 条；
- 距上次成功 Dream 至少 7 天，且消息至少 60 条。

三路条件只控制首次触发。成功批次若因字符或 Session 数上限只处理了连续前缀，会把 `truncated`
保存为明确的待续批状态；后续普通聊天调度仍遵守基础冷却和租约，但直接从成功检查点后领取下一批，
不重新累计 30/50/60 条消息。每次调度最多执行一个批次；最后一批处理完全部积压后清除待续批状态，
之后的少量新消息重新受三路首次门槛控制。门槛不足、执行失败或租约冲突都不推进成功检查点；只有
完成事务提交才更新检查点、待续批状态和上次成功时间。
...
输入的 `SessionMessage.message_id`；同一 Session 的剩余消息和其他连续尾批由后续普通聊天调度逐批
继续，不再受三路首次触发门槛限制。模型或解析失败、租约变化、写入或事务失败时，检查点和已有
待续批状态均保持不变。首条单独消息超限时完整纳入并允许本批临时突破字符上限，检查点推进到该
消息，后续消息留到下一批，不引入字符偏移检查点。
