...
* **分域记忆与共享群聊身份隔离版本**：本版本建立 Memory v3 的个人记忆、群内个人画像和群公共记忆边界，按场景与可见性安全召回；明确记忆请求可在服务端校验后直接保存。同时为共享群聊历史补充发言人归属，降低不同成员的昵称、偏好、身份声明和操作结果发生串线的风险。
...
* **自然语言记忆写入与分域管理**（PR #484 #487）：新增服务端白名单 `save_memory` Tool；用户明确要求“记住”或使用 `/memory 内容` 时，在范围、权限和敏感信息校验通过后直接持久化。支持个人记忆、当前群画像和群公共记忆的列表、详情、编辑、删除、清空及画像停用/启用，群公共写入继续要求明确命令和管理员权限。
...
* 新增记忆不再二次确认；清空记忆、停用群画像以及现有编辑/删除等破坏性管理操作仍按服务端流程确认。普通聊天不会自动写长期记忆，模型文案也不能代替真实持久化结果。
