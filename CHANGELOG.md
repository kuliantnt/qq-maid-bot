...
* **多入口消息可靠性与周易命令易用性**：修复微信服务号客服补发长文本时整轮回复丢失和多轮回复交错的问题，并补齐周易起卦的中文别名与带参数提示。

### Added

* **微信客服长文本安全分片**（PR #690）：客服文本按 `text.content` 的 2048 个 UTF-8 字节上限、Unicode 字符边界顺序切分；每个分片成功后才发送下一片，失败时停止后续发送并保留真实错误。
* **周易起卦别名与带参提示**（PR #690）：新增 `/起卦`、`/卜卦` 中文别名并保留 `/算卦`、`/iching` 入口；带问题文本时不把问题送入起卦流程或回显原文，而是提示用户默念后重新发送无参数命令。

### Changed

* **微信客服补发并发边界**（PR #690）：同一收件人的整组客服分片使用进程内串行锁，不同收件人仍可并行；客服接口请求增加 10 秒单次超时，空闲收件人锁会及时回收。

### Fixed

* **微信服务号慢回复丢失**（PR #690）：超过微信客服单条文本限制的慢回复不再因接口拒绝而整轮丢失；分片发送失败也不会伪造整轮成功。
* **起卦命令误路由**（PR #690）：起卦别名和带参数形式在进入天气快捷解析、普通 Agent 或 Tool Loop 前完成确定性处理。

### Compatibility

* 根包 `qq-maid-bot` 提升到 `0.24.6`；本次实际变更的 `qq-maid-core` 与 `qq-maid-gateway-rs` 分别提升到 `0.1.30` 与 `0.1.21`，`qq-maid-common` 和 `qq-maid-llm` 版本保持不变。
* 本版本不新增 SQLite migration、配置迁移、必填环境变量或运行时入口。微信客服补发仍需已有的 AppID / AppSecret 配置，正常同步回复、AES 回调和其他平台出站语义保持不变。
* 起卦结果继续写入当前 conversation session 供后续追问，不调用 Provider、不进入 pending 或 Tool Loop；起卦带参数形式不会保存或展示用户问题。

## [v0.24.5] - 2026-08-30

### Release Focus

* **周易起卦命令**：新增固定六轮三钱法起卦能力，在本地计算本卦、动爻和之卦，并把卦辞、译文与注释整理为可复核的回执。

### Added

* **`/算卦`（`/iching`）**（PR #687）：只接受无参数命令，使用固定 `3d2+3` 六轮结果映射到 64 卦，展示六爻、卦名、动爻、变卦、原文、译文、注释和原始数值；六爻皆变时使用乾卦“用九”或坤卦“用六”专文。

### Changed

* **跨版本依赖刷新**：`argon2` 升至 `0.6.0`、`fastembed` 升至 `6.0.2`、Gateway 直依赖的 `quick-xml` 升至 `0.42.0`，并通过 `cargo update --verbose` 刷新根 `Cargo.lock`；`feed-rs` 仍按自身约束使用 `quick-xml 0.41.0`。

### Fixed

* **算卦命令边界**：带参数的 `/算卦` 或 `/iching` 会在命令分派处静默收口，不再误进入天气快捷解析或普通 Agent。
* **QQ C2C 静默回执**：Core 明确抑制的无正文响应不会再被 Gateway 降级成空正文提示或发送额外消息。

### Compatibility

* 根包 `qq-maid-bot` 提升到 `0.24.5`；本次实际变更的 `qq-maid-core` 与 `qq-maid-gateway-rs` 分别提升到 `0.1.29` 与 `0.1.20`，`qq-maid-common` 和 `qq-maid-llm` 版本保持不变。
* 本版本不新增 SQLite migration、配置迁移、必填环境变量或运行时入口；`/算卦` 不调用 Provider，不进入普通 session、pending 或 Tool Loop。Argon2 仍使用默认 Argon2id 参数，已有 PHC 密码哈希继续由验证流程读取。
* 新增的起卦文本为程序内置公开数据；命令仍受当前平台命令前缀配置约束，带参数形式保持静默，不会伪造成功回复。
