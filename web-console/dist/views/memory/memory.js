import { archiveMemory, commitMemoryOperation, createMemory, getMemory, listMemoryTargets, listMemories, prepareMemoryOperation, restoreMemory, updateMemory, } from "../../api.js";
...
function memoryCard(item) {
    ...
    if (item.status === "active") {
        if (item.capabilities.canUpdate)
            actions.append(actionButton("纠正内容", () => void editMemory(item)));
        if (item.capabilities.canArchive)
            actions.append(actionButton("归档", () => void archiveItem(item)));
    }
...
function renderMemoryAdvancedFilters() {
...
async function editMemory(item) {
    try {
        ...
        if (content === null || content.trim() === latest.content)
            return;
        await updateMemory({
...
function renderPagination() {
    ...
    if (memoryListState !== "ready" || currentPage.totalPages <= 1)
        return;
    const previous = actionButton("上一页", () => { if (page > 1) {
        page -= 1;
        void refreshMemories();
    } });
    previous.disabled = page <= 1;
    const next = actionButton("下一页", () => { if (page < currentPage.totalPages) {
        page += 1;
        void refreshMemories();
    } });
...
function clearMemoryPagination() {
    const target = element("memory-pagination", HTMLElement);
...
function setMemoryCreateControlsDisabled(disabled) {
    for (const id of [
...
function isMemoryVisibility(value) {
    return value === "private" || value === "context_only" || value === "group_members" || value === "public";
}
function asMemoryVisibility(value) {
    return isMemoryVisibility(value) ? value : null;
}
function actionButton(label, callback, variant = "secondary") {
...
function element(id, type) {
    ...
    if (!(value instanceof type))
        throw new Error(`Memory 页面缺少控件 ${id}`);
    return value;
}
