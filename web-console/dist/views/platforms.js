...
export function renderPlatforms(platforms) {
    ...
    capabilityBody.replaceChildren(...platforms.flatMap((platform) => platform.capabilityScopes.flatMap((scope) => capabilityRows(platform.label, scope))));
}
...
function capabilityRows(platformLabel, scope) {
    return [
...
function capabilityRow(platformLabel, scopeLabel, direction, capabilities) {
    const row = document.createElement("tr");
    row.append(cell(platformLabel), cell(scopeLabel), cell(direction), capabilityCell(capabilities.text), capabilityCell(capabilities.markdown), capabilityCell(capabilities.image), capabilityCell(capabilities.file), capabilityCell(capabilities.mixedMessage), capabilityCell(capabilities.streaming));
    return row;
}
