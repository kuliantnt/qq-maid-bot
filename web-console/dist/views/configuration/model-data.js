...
export function mergeModels(discovery, metadata) {
    const rows = new Map();
...
export function discoveryLabel(result) {
    if (!result)
        return "尚未获取模型（unknown）";
    const labels = { success: array(result.models).length ? `获取成功：${array(result.models).length} 个模型` : "获取成功：Connection 返回空列表", unsupported: "不支持模型发现（unsupported）", failed: "获取失败（failed）", unknown: "无法确认模型列表（unknown）" };
    return `${labels[string(result.state)] ?? labels.unknown} · ${result.category ?? ""}`;
}
export function filterModels(rows, query, status) {
