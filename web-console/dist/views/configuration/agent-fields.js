...
import { array, checkboxField, configurationGroup, element, fieldGroup, numberField, record, selectField, sourceLabel, statusField, string, textField } from "./fields.js";
...
export const AGENT_ROUTE_LABELS = {
    private_main: "私聊主路线",
    group_main: "群聊主路线",
    aux: "辅助任务路线",
...
export function renderAgent(snapshot) {
...
export async function saveAgentScene(sceneName) {
    if (!current?.agent)
        return;
...
export async function saveOpenCodeProvider(id) {
    if (!current?.agent)
        return;
...
export function agentSceneConfig(sceneName, scenes) {
...
export function toolCheckbox(tool, sceneName) {
    const label = document.createElement("label");
