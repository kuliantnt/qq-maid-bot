...
export function appendModelManager(parent, connection, revision, enabled) {
    ...
    open.onclick = () => {
        ...
        const save = async (entry, expectedRevision = string(metadata.revision)) => {
            const result = await runSave(async () => {
                try {
                    return await updateModelOverride(connection, expectedRevision, entry);
                }
                catch (cause) {
            ...
            if (!result)
                return false;
            metadata = await fetchModelMetadata(connection);
            render();
            status.textContent = "本地模型已保存；重启后生效。";
            return true;
        };
        const render = () => {
            ...
            if (!rows.length)
                list.append(node("p", "当前没有匹配的展示条目；Connection 获取状态见上方。"));
            ...
            for (const row of rows.slice(0, visibleLimit)) {
                ...
                detail.append(node("summary", "模型信息 / Advertised / Provenance"));
                ...
                button(card, "编辑模型信息", false, async () => {
                    formRevision = string(metadata.revision);
                    form.edit(row.id, row.override);
                    editor.open = true;
                ...
                route.setAttribute("aria-label", `为 ${row.id} 选择 Route`);
                ...
                button(card, "加入 Route", !enabled || row.metadata.enabled === false || !current?.agent?.editable || !Object.keys(routes).length, async () => {
                    if (row.id.includes(","))
                        throw new Error("该模型 ID 含有路线分隔符，无法按现有语法加入 Route");
                    ...
                    if (saved)
                        status.textContent = `已加入 Route ${route.value}；重启后生效。`;
                }, error);
                list.append(card);
            }
        };
        button(editor, "保存本地模型", false, async () => {
            if (!loaded)
                throw new Error("模型信息尚未加载成功");
            if (await save({ ...form.read(), provider: metadata.provider }, formRevision))
                formRevision = string(metadata.revision);
        }, error);
        ...
        button(controls, "获取模型", !enabled, async () => {
            status.textContent = "正在获取 Connection 模型…";
            discovery = null;
            render();
            try {
                discovery = await discoverConnectionModels(connection, revision);
            }
            catch (cause) {
                discovery = { state: "failed", category: errorMessage(cause) };
            }
            status.textContent = discoveryLabel(discovery);
            render();
        }, error);
        ...
        search.oninput = () => { visibleLimit = 50; render(); };
...
function field(label) { const input = node("input", ""); input.placeholder = label; input.setAttribute("aria-label", label); return input; }
function button(parent, label, disabled, action, error) {
    const element = node("button", label);
    element.type = "button";
    element.className = "secondary provider-action";
    element.disabled = disabled;
    element.dataset.modelDisabled = String(disabled);
    element.onclick = async () => { element.disabled = true; error.textContent = ""; try {
        await action();
    }
    catch (cause) {
        error.textContent = errorMessage(cause);
...
function appendModelDetails(parent, model, source) {
    ...
    const labels = { catalog: "目录", official_patch: "官方补丁", local_override: "本地覆盖" };
    const row = (label, value, origin) => {
        parent.append(node("p", `${label}：${value ?? "未知"}${typeof origin === "string" ? ` · 来源：${labels[origin] ?? origin}` : ""}`));
    };
    for (const [key, label] of [["display_name", "显示名称"], ["context_window", "上下文窗口"], ["max_output_tokens", "最大输出"], ["status", "状态"]])
        row(label, model[key], provenance[key]);
    ...
    if (Object.keys(source).length) {
        ...
        details.append(node("summary", "目录出处"));
        for (const [key, label] of [["name", "数据源"], ["source_url", "上游地址"], ["source_version", "版本"], ["fetched_at", "快照时间"], ["upstream_license", "许可"], ["source_hash", "校验摘要"], ["converter_version", "转换版本"]])
            details.append(node("p", `${label}：${source[key] ?? "未知"}`));
