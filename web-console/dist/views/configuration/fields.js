...
export function appendGroupedFields(target, fields, row, source) {
...
export function configurationGroup(group, source, content) {
...
export function fieldGroup(label, rows, description) {
...
export function inputValue(input, field) {
    if (field.valueType === "boolean")
...
export function configInput(key) {
...
export function textField(labelText, id, value, disabled, autosaveScope) {
...
export function numberField(labelText, id, value, min, max, disabled, autosaveScope) {
    const row = document.createElement("div");
    row.className = "config-row";
    const label = document.createElement("label");
    label.htmlFor = id;
...
export function selectField(labelText, id, value, options, disabled, autosaveScope) {
    const row = document.createElement("div");
    row.className = "config-row";
    const label = document.createElement("label");
    label.htmlFor = id;
...
export function checkboxField(labelText, id, checked, disabled, autosaveScope) {
    const row = document.createElement("div");
...
export function statusField(summary, detail) {
    const row = document.createElement("div");
    row.className = "config-row";
...
export function button(text, kind) {
...
export function inputId(key) { return `config-${key.replaceAll(".", "-")}`; }
...
export function positiveNumber(value, fallback) { return typeof value === "number" && Number.isFinite(value) && value > 0 ? value : fallback; }
export function integerInput(id) { return Number(element(id, HTMLInputElement).value); }
export function element(id, constructor) {
