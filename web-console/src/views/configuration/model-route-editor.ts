...
export interface ModelRouteEditorOptions {
  label: string;
  inputId: string;
  candidates: string[];
  disabled: boolean;
...
export function renderModelRouteEditor(options: ModelRouteEditorOptions): HTMLElement {
  ...
  const renderChips = (): void => {
    ...
    list.forEach((candidate, index) => {
      ...
      chip.addEventListener("drop", (event) => {
        event.preventDefault();
        chip.classList.remove("model-route-chip--drag-over");
        const from = Number(chips.querySelector<HTMLElement>(".model-route-chip--dragging")?.dataset.dragFrom ?? -1);
      ...
      if (!options.disabled) chip.append(handle);
