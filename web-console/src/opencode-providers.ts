...
export interface OpenCodeProviderForm {
  id: string;
  label: string;
...
const PRESETS: ReadonlyArray<OpenCodeProviderForm> = [
  {
    id: "opencode_zen",
...
export function openCodeProviderPresets(): OpenCodeProviderForm[] {
  return PRESETS.map((preset) => ({ ...preset }));
}

export function readOpenCodeProviders(documentValue: unknown): OpenCodeProviderForm[] {
  const providers = record(record(documentValue).providers);
  return PRESETS.map((preset) => {
    const saved = record(providers[preset.id]);
    if (Object.keys(saved).length === 0) return { ...preset };
...
export function openCodeProviderChange(form: OpenCodeProviderForm): Record<string, unknown> {
  const preset = PRESETS.find((value) => value.id === form.id);
  if (!preset || form.kind !== preset.kind) throw new Error("OpenCode Provider ID 或协议不受支持");
  if (!isHttpUrl(form.baseUrl)) throw new Error(`${form.label} Base URL 必须是合法的 HTTP(S) 地址`);
  if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(form.apiKeyEnv)) throw new Error(`${form.label} API Key 环境变量名无效`);
  if (!form.authHeader.trim()) throw new Error(`${form.label} 认证 Header 不能为空`);
  ...
  if (form.kind === "openai_responses") provider.chat_fallback = form.chatFallback;
  return { action: "set_provider", id: form.id, provider };
}

export function openCodeProviderWarning(enabled: boolean, keyConfigured: boolean): string {
  if (!enabled && !keyConfigured) return "Provider 未添加，OpenCode API Key 也未配置；仍可先编辑模型路线。";
  if (!enabled) return "Provider 尚未添加；仍可先编辑模型路线。";
  if (!keyConfigured) return "Provider 已保存，但 OpenCode API Key 尚未配置。";
  return "";
}

export function renderOpenCodeProviders(
  snapshot: ConfigurationSnapshot,
  onSave: (form: OpenCodeProviderForm) => Promise<void>,
...
export function renderOpenCodeRouteHints(
  disabled: boolean,
  ...
  for (const [example, provider] of examples) {
    ...
    for (const [route, label] of [["private_main", "插入私聊"], ["group_main", "插入群聊"], ["aux", "插入辅助"]] as const) {
...
function providerCard(
  form: OpenCodeProviderForm,
  running: OpenCodeProviderForm,
  keyConfigured: boolean,
  editable: boolean,
  onSave: (form: OpenCodeProviderForm) => Promise<void>,
  onRemove: (id: string) => Promise<void>,
  ...
  if (form.enabled) {
    ...
    remove.onclick = () => {
      if (window.confirm(`确定移除 ${form.label} 吗？已有模型路线不会自动删除。`)) void onRemove(form.id);
    };
...
function readProviderForm(
  source: OpenCodeProviderForm,
  baseUrl: HTMLInputElement,
  apiKeyEnv: HTMLInputElement,
  authHeader: HTMLInputElement,
  authScheme: HTMLInputElement,
  timeout: HTMLInputElement,
  card: HTMLElement,
): OpenCodeProviderForm {
  const fallback = card.querySelector<HTMLInputElement>(`#${source.id}-chat-fallback`);
  return {
    ...source,
...
function textInput(labelText: string, id: string, value: string, disabled: boolean, type = "text"): { row: HTMLElement; input: HTMLInputElement } {
...
function checkboxInput(labelText: string, id: string, checked: boolean, disabled: boolean): { row: HTMLElement; input: HTMLInputElement } {
  const row = document.createElement("label");
...
function appendRouteCandidate(id: string, candidate: string): void {
