...
| `runtime/config/agent.example.toml` | 场景、模型候选链、profile、Tool Loop 预算和工具白名单等 Agent 策略模板；首次启动生成本地 `config/agent.toml` |
