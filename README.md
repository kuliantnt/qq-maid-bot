...
- `llm/`：服务器部署目录，保留 release 二进制、运行配置和运行产物。
...
公开仓库只包含源码和 `.example` 模板。复制 `.env.example` 后，按需填写模型、QQ 和天气配置；未显式配置 `PROMPT_DIR` 时，LLM 会在缺少私有 prompt 文件的情况下使用内置通用 prompt 启动。私人 prompt、世界观、成员映射和运行数据建议放在外部私有目录，再通过 `.env` 中的路径变量注入。

编辑根目录 `.env` 后，先启动 Rust LLM 服务：
...
`llm/` 是服务器运行目录。执行 `make build` 只会构建本地 release 二进制；发布到服务器可从仓库根目录执行：

```bash
./scripts/deploy.sh
```

脚本会构建 release 二进制、上传到远端 `llm/` 目录，并重启远端服务。远端运行目录结构：
...
服务器上可把真实 `.env` 放到 `llm/.env` 或 `llm/config/.env`，并在其中把 `PROMPT_DIR`、`MEMBER_ID_MAPPING_FILE`、`WORLD_FILE`、`APP_DB_FILE` 指向外部私有配置或运行数据目录，再执行：
...
- 本地统一配置入口：仓库根 `.env`，模板见根 `.env.example`。
- 模块兼容配置入口：`qq-maid-gateway-rs/config/.env`、`qq-maid-llm/config/.env`。
- 部署目录配置：`llm/.env` 或 `llm/config/.env`。

`scripts/llmctl.sh` 和 `scripts/gatewayctl.sh` 部署后会复制为远端 `llm/llmctl.sh` 与 `llm/gatewayctl.sh`。控制脚本当前只 source 第一个存在的配置文件：显式 `LLM_ENV_FILE` / `GATEWAY_ENV_FILE` 优先，其次是仓库根 `.env`、`llm/.env`、`llm/config/.env` 与对应模块 `config/.env`。Rust 进程自身还会按当前工作目录尝试加载 `config/.env` 再加载 `.env`；`dotenvy` 默认不覆盖已存在的环境变量，所以进程环境变量优先，先加载的 dotenv 文件会保留同名变量，后续文件只补充缺失项。
...
相对路径按进程启动工作目录解析。本地 `make run-llm` 通常以仓库根目录为工作目录；部署脚本启动时会 `cd llm` 后运行二进制。

不要读取、打印或提交真实 `.env`，也不要把 token、secret、API Key、bot appid、私钥、真实 QQ 群聊或私聊内容、openid、群 ID、用户数据写进文档和代码。Secret、SQLite 数据库、日志、pid 文件和聊天记录不应进入任何 Git 仓库；真实 prompt、世界观和成员映射只应放在私有配置仓库或本地私有目录，不进入公开仓库。
