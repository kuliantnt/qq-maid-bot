...
- **通用骰子表达式扩展**（Issue #682 Phase 1）：支持 `1d20+3`、`2d6+1`、`1d8+1d6+4` 等带修正和多段骰子表达式，新增 `/r` 作为 `/roll` 短别名，并允许 `/r 2d20 问题` 使用指定骰式进行 AI DM 判定；Core 先确定性计算规范骰式和理论总值范围，AI DM 只返回难度，Core 再按娱乐模式区间刻度（包括默认 `1d20`，不是正式 DND5E 规则）计算 DC、生成骰值并结算。解析、随机和结算均在 Core 本地完成，并对表达式复杂度设有限制。
