...
- **记忆与知识库**：个人记忆、群内个人画像和群公共记忆分域管理，并按场景与可见性召回。用户明确要求“记住”时可直接保存；可选的 Session Dream 默认关闭，启用后仅将安全的普通聊天长期事实沉淀为 Personal 或当前成员 GroupProfile；本地 Markdown 可自动索引并按需检索。
