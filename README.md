...
当前源码版本线：**v0.15.1：Luna 路线与普通消息收口版本**。这一版将默认 Agent 模型路线统一到 OpenAI GPT-5.6 Luna，让普通消息的路由规划、流式执行、状态提示和诊断共用同一份决策，并新增 Linux `qbot` 一键安装与管理脚本。历史版本记录见 [CHANGELOG.md](./CHANGELOG.md)，实际发布包以 [Releases](https://github.com/kuliantnt/qq-maid-bot/releases) 页面为准。

## 当前版本亮点

| 重点 | 说明 |
| --- | --- |
| Luna 统一主路线 | 默认 `agent.toml` 的私聊、群聊和辅助任务都以 OpenAI GPT-5.6 Luna 为第一候选，并保留 Gemini、MiMo 和 DeepSeek 自动降级。 |
| 一键安装与管理 | Linux 用户可通过 `qbot.sh` 安装 Release、配置 QQ Bot 与 AI 渠道，并使用 `qbot` 命令启停、升级、查看日志和健康检查。 |
| 普通消息统一路由 | Router 在规划阶段生成单次不可变决策，Core streaming、dispatcher、chat flow、状态提示和诊断共享结果，降低同一请求在不同阶段路由漂移的风险。 |
...
默认 `runtime/config/agent.toml` 会随 runtime 和 Release 包分发，其私聊、群聊和辅助任务都以 OpenAI GPT-5.6 Luna 为第一候选，并保留 Gemini、MiMo 和 DeepSeek 降级候选；搜索路线默认使用 Luna。默认主路线需要 `OPENAI_API_KEY` 且 API 账号需有 `gpt-5.6-luna` 访问权限，未配置对应 Key 的后备 Provider 会被跳过。如需调整候选顺序或搜索 Provider，请修改 `agent.toml` 中的 `model_routes` / `search_routes`；`.env` 中的兼容模型变量仅在对应 route 未显式声明时生效。
...
适合 Linux 服务器。脚本通过 `curl` 获取，不需要先 `git clone` 仓库；默认安装到 `/root/qq-maid-bot`，并部署 `qbot` 管理命令。
...
以后升级可直接执行 `qbot update`；查看日志和健康检查可用 `qbot log`、`qbot health`。

### 路径二：Linux Release 包（推荐，无需安装 Rust）
...
最少需要填写：`QQ_BOT_APP_ID`、`QQ_BOT_APP_SECRET`，以及默认候选链中至少一个 Provider 的 API Key。优先使用 Luna 时填写 `OPENAI_API_KEY` 并确认账号已获得 `gpt-5.6-luna` API 访问权限；也可配置 Gemini、MiMo 或 DeepSeek Key 启用后备路线。使用第三方或自建兼容接口时，需同步修改 `agent.toml` route 和 `.env` 中的 API Key / Base URL。需要处理 QQ 图片时，保持 `QQ_MAID_ENABLE_IMAGE=true`，并确认模型路线使用支持图片输入的 provider / model。
...
当前源码版本线为 **v0.15.1：Luna 路线与普通消息收口版本**；已发布稳定包请以 [Releases](https://github.com/kuliantnt/qq-maid-bot/releases) 页面为准。版本升级前请先阅读 [CHANGELOG.md](./CHANGELOG.md)，并对比新版 `runtime/config/.env.example` 和 `runtime/config/agent.toml`。
