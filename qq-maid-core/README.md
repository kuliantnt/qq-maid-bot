...
- 娱乐：`/roll`（别名 `/r`、`/rd`）无参数时本地掷 D20，常用表达式、`N#expr` 多轮、`k/q` 取骰和优势/劣势也在 Core 本地解析、掷骰和结算；`/r d50 开锁`、`/r2d6xxx` 的尾随文本仅作为骰点原因展示，不调用模型。奖励/惩罚骰可用 `/rab` / `/rap`，玩家展示名沿用 `/set 昵称`，也可用 `/nn emmm`（默认前缀下兼容 `.nn emmm`）。带自然语言问题的 `/roll <骰子表达式> <问题>` 保留为显式 Entertainment DM，AI 只选择 difficulty，Core 再按统一的娱乐模式区间刻度计算 DC 并掷骰；本地骰点、原因和多轮路径都不进入普通 session 或 Tool Loop，也不调用模型。
