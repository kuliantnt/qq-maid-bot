...
仅当 `WEB_CONSOLE_ENABLED=true` 时注册。新实例没有 Provider 或平台凭据时进入 `setup_required`，仍保留 `/healthz` 和受保护的 `/console/`，但不构造业务 Provider、Worker 或 Gateway。首次启动会在主密钥同目录生成短时、单次的 `bootstrap.token`；默认只写入权限受限文件，不输出完整 token，用户建立首位部署管理员后令牌立即失效。`WEB_CONSOLE_LOG_BOOTSTRAP_TOKEN=true` 才会启用控制台显示，可能造成日志持久化泄露，生产环境必须关闭。

配置详情和写接口要求独立于聊天 session 的部署管理员服务端会话。密码使用 Argon2id 哈希；登录/初始化受限流、SameSite HttpOnly cookie、CSRF、会话轮换/过期和脱敏审计保护。普通字段写入受管 `runtime.toml`，secret 认证加密写入 SQLite，Agent 路线与 Tool Calling 结构化修改现有 `agent.toml`。所有修改携带 revision；冲突不会覆盖人工新文件。分步首次向导允许保存整体尚未完成的配置，但页面和 `/healthz` 会继续报告 `setup_required`，直至重启后完整预检通过。

状态接口只读取已有配置、文件元数据和进程内观测，不调用 Provider、不探测外网。配置页的“安全校验”复用正式启动预检；只有管理员显式点击“受控测试”时，服务端才会向所选官方 HTTPS Provider 的模型列表端点发起 8 秒内的无聊天内容请求。自定义地址不允许从该入口探测，避免形成任意 URL/内网 SSRF 能力。Markdown 渲染接口限制请求体 64 KiB，并使用 HTML sanitizer 清理脚本、事件属性和危险链接。
