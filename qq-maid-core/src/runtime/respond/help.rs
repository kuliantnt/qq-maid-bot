...
const HELP_MODULES: &[HelpModule] = &[
    ...
    HelpModule {
        ...
        aliases: &["记忆"],
        title: "🧠 长期记忆",
        summary: "明确指令先生成记忆草稿，只有当前操作者确认后才会写入。中文别名：`/记忆`、`/记`。",
        commands: &[
            "- `/memory personal 内容`：保存个人记忆；旧写法 `/memory 内容` 仍兼容",
            "- `/memory profile 内容`：保存你在当前群的个人画像",
            "- `/memory group add 内容`：保存当前群公共记忆（仅管理员；旧关键词搜索仍兼容）",
            "- `/memory [personal|profile|group] list [关键词]`：按范围查看或搜索",
            "- `/memory show 序号`、`/memory edit 序号 新内容`、`/memory delete 序号`、`/memory clear`：管理最近列表",
            "- `/memory profile stop|enable`：停止或重新授权当前群画像写入",
        ],
        notes: &[
            "- 普通聊天不会自动写长期记忆；新增、修改、删除、清空和画像授权变更都需要确认。",
