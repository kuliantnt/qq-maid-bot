...
const HELP_MODULES: &[HelpModule] = &[
    ...
    HelpModule {
        ...
        commands: &[
            ...
            "- `/ri 12 张三, +2 李四, d20+4 王五`：录入先攻；`/ri+5 哥布林1，+2 哥布林2` 是 `/ri +5 哥布林1，+2 哥布林2` 的 SealDice 紧凑写法，`/ri优势+4 哥布林` 等价 `/ri 优势+4 哥布林`，默认前缀下也可用 `.ri+5`、`.ri优势+4`；裸骰式省略名称时沿用 `/nn` 或平台展示名；`/init` 查看；`/init set 单位 骰式` 设置；`/init del 单位` 删除；`/init end` 推进；`/init clr` 清空。先攻临时保存，重启清空。",
