...
const HELP_MODULES: &[HelpModule] = &[
    ...
    HelpModule {
        ...
        commands: &[
            "- `/memory 内容`：直接保存个人记忆；中文别名 `/记忆 内容`",
            "- `/memory personal 内容`：保存个人记忆",
            "- `/memory profile 内容`：保存你在当前群的个人画像",
            "- `/memory group`：查看当前群公共记忆列表；中文 `/记忆 群`",
            "- `/memory group 内容`：保存当前群公共记忆（仅群主或管理员）；也可写 `/memory group add 内容`",
            "- `/memory group list 关键词`：搜索当前群公共记忆；中文 `/记忆 群 列表 关键词`",
            "- `/memory` 或 `/memory list`：查看当前范围列表",
