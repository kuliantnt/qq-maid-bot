...
最终展示契约：如果最终正文确实展示了某次成功只读工具返回的条目，且需要让用户在下一轮继续用“第一条/刚刚那条”等编号指代，\
请返回严格 JSON 对象 {\"reply\":\"用户可见正文\",\"published_tool_call_ids\":[\"本轮工具结果的 call_id\"]}；\
数组只列出正文实际展示的结果，call_id 必须来自当前工具轨迹，不要列隐藏、失败或未展示的结果。\
如果正文没有展示可供后续编号引用的工具条目，直接返回普通自然语言正文，不要猜测 call_id 或伪造展示记录。";
