...
use qq_maid_llm::tool::ToolContext;
...
/// 从 `RespondRequest` 生成服务端 ToolContext；不允许模型提供 user/scope。
///
/// 该函数同时被 Agent Tool Loop 与确定性 Todo 短路路径复用，保证两侧的
/// actor / scope 语义完全一致。
pub(crate) fn tool_context_from_request(req: &RespondRequest) -> ToolContext {
...
pub(crate) fn tool_conversation_from_request(
    req: &RespondRequest,
) -> (ConversationKind, Option<String>) {
