...
//!
//! 核心职责：
//! - 从外部 JSON 文件加载成员编号映射（支持字符串或对象两种值格式）；
//! - 在用户消息中通过正则识别成员编号自称（如“我是407”）；
//! - 生成供 LLM 理解的成员身份提示文本（注入 system prompt）；
//! - 未知编号的纠错提示（按末两位后缀匹配相似编号）。
//!
//! 约束：
//! - 成员编号必须是 3 位数字且首位非 0（`is_member_id`）；
//! - 缺失映射文件视为空映射，不视为错误（兼容无映射场景）；
//! - JSON 解析失败是硬错误（防止静默丢失配置）。
