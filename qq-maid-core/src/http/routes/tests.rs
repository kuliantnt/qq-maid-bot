...
async fn request_text_response(
    ...
    let (status, headers, body) = request_raw_response(state, method, path, value, accept).await;
...
async fn console_routes_are_not_registered_by_default() -> Result<(), Infallible> {
    let (console_status, _json) = request_response(test_state(), "GET", "/console/", None).await;
    ...
    .await;
    let (css_status, _) = request_response(test_state(), "GET", "/console/styles.css", None).await;
...
async fn bootstrap_status_is_read_only_and_preauth_uses_a_separate_cookie() {
    ...
    let (database, directory) =
        SqliteDatabase::open_temp_directory("qq-maid-bootstrap-cookie-separation", APP_MIGRATIONS)
            .unwrap();
    state.admin_auth =
        Some(AdminAuth::open(database, directory.join("config/secrets/bootstrap.token")).unwrap());
...
#[tokio::test]
async fn console_cors_preflight_allows_json_post_for_configured_origin() -> Result<(), Infallible> {
    let mut state = test_state();
    state.config.web_console_enabled = true;
    state.config.web_console_allowed_origins = vec!["https://console.example".to_owned()];
    let app = build_router(state);
...
#[tokio::test]
async fn provider_connection_test_route_is_not_registered() -> Result<(), Infallible> {
    let mut state = test_state();
    state.config.web_console_enabled = true;
    let (status, _json) = request_response(
        state,
